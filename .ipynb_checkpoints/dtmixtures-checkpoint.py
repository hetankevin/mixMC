...
class Distribution():
    """A distribution over trails of length 3
    """
    def __init__(self):
        self.n = None
        self.t_len = None
    ...
    def from_trails(n, trails):
        ...
        trails = trails[:, :3 * (t_len // 3)].reshape((-1, 3))
...
class Mixture():
    """A discrete-time mixture of Markov chains.
    """
    def __init__(self, S, Ms):
        """Initialize a mixture of Markov chains.
        
        L: number of chains
        n: number of states
        S: L x n matrix of initial state probabilities
        Ms: L x n x n matrix of transition probabilities
        """
    ...
    def max_mixing_time(self, eps=1e-10):
        def mixing_time(s, M):
            ...
            thigh = None
            t = tlow
            while thigh is None or thigh > tlow + 1:
                if thigh is None:
                    t *= 2
                else:
                    t = (tlow + thigh) // 2
                # print(tlow, "<", t, "<", thigh)
                y = s @ np.linalg.matrix_power(M, t)
        ...
        return max([mixing_time(s, M) for s, M in zip(self.S, self.Ms)])
